...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        k = await query.message.edit('This Movie Not Found In channel,check whether the Movie is Released or not..'')
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        cap ={imdb['url']}>{imdb.get('title')}</a>\n🎭 Genres: {imdb.get('genres')}\n📆 Year: <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a>\n🌟 Rating: <a href={imdb['url']}/ratings>{imdb.get('rating')} </code>"
    else:
